...
module.exports = (connection) => {
  router.get('/', (req, res) => {
    const { start, end, doctorId } = req.query; // Récupérer les dates de début et de fin, et l'ID du médecin

    // Validation des dates et de l'ID du médecin
    if (!start || !end || !doctorId) {
      res.status(400).json({ error: 'Start date, end date, and doctorId are required' });
      return;
    }
